...
The parsing tests run against `fixtures/confirm-email.eml`. It began as a real
message captured from the live service and carried across from the TypeScript
driver; it has since been **sanitised for publication** — hosts, addresses,
message ids, the token and the sender's identity are synthetic.

What was deliberately preserved is the only part that matters here: the message
still wraps its own link at two *different* awkward places. The `text/plain`
part soft-breaks inside the **hostname**, the `text/html` part soft-breaks
inside the **token** at 24 of its 32 characters. Both parsing bugs these tests
pin (the quoted-printable split and the truncating URL bound) are invisible
against a fixture that doesn't do that, which is why it was structure-preserved
rather than regenerated by hand.

So it is no longer *captured*, it is *constructed to match a capture* — a
slightly weaker guarantee, and stated here rather than implied.
"""
...
class TestQuotedPrintable:
    ...
    def test_real_message_yields_an_intact_host(self, confirm_email):
        ...
        assert "example.=" not in decoded, "soft break left in place — the host is truncated"
...
class TestFixtureIntegrity:
    def test_captured_message_has_not_drifted(self):
        ...
        # 🔴 The shared baseline is GONE, deliberately. Until 2026-08-17 this copy was
        # byte-identical to portal-client/src/test/fixtures/ and
        # test-hub/src/drivers/mail/fixtures/, and the pin's job was to catch the three
        # drifting apart. This repo is PUBLIC and that fixture was a real captured
        # message — internal hostnames, a real recipient, provider message ids and a
        # live confirmation link — so this copy was sanitised and now differs from its
        # siblings ON PURPOSE.
        #
        # The pin therefore no longer proves cross-repo agreement; it only catches
        # accidental local edits. Cross-copy agreement now has to be argued from the
        # preserved STRUCTURE (both soft-break positions), not from a digest — and
        # nothing automated checks that. Named gap, not a solved problem.
        digest = hashlib.sha256(FIXTURE.read_bytes()).hexdigest()
        assert digest == "e4e670f91e3f4f4a2cd50627bbc3265bd270c5190acfb9d40ee2b904add2d017", (
            "confirm-email.eml changed. If that was deliberate, re-pin this digest — and "
            "re-check that BOTH soft breaks survived, since that is all this fixture is "
            "for: the text/plain break inside the hostname and the text/html break inside "
            "the token."
        )
