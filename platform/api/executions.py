"""Execution list/detail/cancel + chat endpoints."""
...
# ── Chat endpoints (nested under /api/v1/workflows) ───────────────────────────
...
def get_chat_history(
    ...
    for msg in messages:
        ...
        # Fallback: inherit from the preceding message
        ...
        if msg.type == "human":
            all_messages.append(
                ChatHistoryMessageOut(role="user", text=content, timestamp=timestamp)
            )
        elif msg.type == "ai" and content:
            # Skip empty AI messages (often tool calls)
            all_messages.append(
                ChatHistoryMessageOut(
                    role="assistant", text=content, timestamp=timestamp
                )
            )
